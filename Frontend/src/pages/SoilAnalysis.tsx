...
const SoilAnalysis: React.FC = () => {
  ...
  const analyzeSoil = (data: ExtendedSoilData): AnalysisResult => {
    ...
    if (data.organicMatter < 1.0) {
      enhancedRecommendations.push(
        "कार्बनिक पदार्थ कम है। हरी खाद और कम्पोस्ट का प्रयोग बढ़ाएं।",
      );
    } else if (data.organicMatter > 3.0) {
      enhancedRecommendations.push(
        "कार्बनिक पदार्थ अधिक है। नाइट्रोजन उर्वरक कम करें।",
      );
    }
    ...
    if (data.electricalConductivity > 2.0) {
      enhancedRecommendations.push(
        "मिट्टी में लवणता अधिक है। जल निकासी सुधारें और जिप्सम डालें।",
      );
    } else if (data.electricalConductivity < 0.3) {
      enhancedRecommendations.push(
        "मिट्टी में लवणता कम है। सूक्ष्म पोषक तत्वों का प्रयोग करें।",
      );
    }
    ...
    if (data.zinc < 0.5) {
      enhancedRecommendations.push(
        "जिंक की कमी है। जिंक सल्फेट का प्रयोग करें।",
      );
    }
    ...
    if (data.iron < 5) {
      enhancedRecommendations.push(
        "लोहे की कमी है। फेरस सल्फेट का प्रयोग करें।",
      );
    }
    ...
    if (data.magnesium < 30) {
      enhancedRecommendations.push(
        "मैग्नीशियम की कमी है। मैग्नीशियम सल्फेट का प्रयोग करें।",
      );
    }
    ...
    if (data.sulfur < 10) {
      enhancedRecommendations.push(
        "सल्फर की कमी है। जिप्सम या अमोनियम सल्फेट का प्रयोग करें।",
      );
    }
    ...
    if (data.soilTexture === "clay") {
      enhancedRecommendations.push(
        "चिकनी मिट्टी है। जल निकासी सुधारें और रेत मिलाएं।",
      );
    } else if (data.soilTexture === "sandy") {
      enhancedRecommendations.push(
        "बलुई मिट्टी है। कार्बनिक पदार्थ बढ़ाएं और सिंचाई बार-बार करें।",
      );
    }
    ...
    if (data.drainage === "poor") {
      enhancedRecommendations.push(
        "जल निकासी खराब है। नालियां बनाएं और जिप्सम डालें।",
      );
    }
    ...
    if (data.slope > 5) {
      enhancedRecommendations.push(
        "ढलान अधिक है। कंटूर खेती और टेरेसिंग अपनाएं।",
      );
    }
    ...
    if (data.tillagePractice === "conventional" && data.organicMatter < 1.5) {
      enhancedRecommendations.push(
        "न्यूनतम जुताई अपनाएं ताकि कार्बनिक पदार्थ बचा रहे।",
      );
    }
  ...
  function getCropRequirements(
    ...
    let req = CROP_REQUIREMENTS.find(
      (r) =>
        r.cropId === cropId && r.stateId === stateId && r.district === district,
    );
  ...
  function validateForm(data: ExtendedSoilData) {
    if (!data.nitrogen || isNaN(Number(data.nitrogen)))
      return "Invalid nitrogen value";
    if (!data.phosphorus || isNaN(Number(data.phosphorus)))
      return "Invalid phosphorus value";
    if (!data.potassium || isNaN(Number(data.potassium)))
      return "Invalid potassium value";
    if (!data.ph || isNaN(Number(data.ph)) || data.ph < 0 || data.ph > 14)
      return "Invalid pH value";
    if (!data.plotArea || isNaN(Number(data.plotArea)) || data.plotArea <= 0)
      return "Invalid plot area";
    if (!data.state || !data.district || !data.crop)
      return "Please select state, district, and crop";
    return null;
  }
  ...
  function calculateResults(data: ExtendedSoilData) {
    ...
    const cropReq = getCropRequirements(
      Number(data.crop),
      Number(data.state),
      data.district,
    );
    ...
    if (data.ph < cropReq.min_ph) {
      ...
      phStatus = "Alkaline";
      phRecommendation =
        "Apply agricultural sulfur or gypsum to decrease soil pH.";
    }
    ...
    if (data.organicMatter < 1.0) {
      ...
      organicMatterRecommendation =
        "Apply green manure, compost, or farmyard manure to increase organic matter.";
    } else if (data.organicMatter > 3.0) {
      organicMatterStatus = "High";
      organicMatterRecommendation =
        "Organic matter is high. Reduce nitrogen fertilizer application.";
    }
    ...
    if (data.electricalConductivity > 2.0) {
      ...
      ecStatus = "Low";
      ecRecommendation =
        "Low soil salinity. Consider micronutrient application.";
    }
    ...
    if (data.organicMatter < 1.0) {
      soilHealthScore -= 20;
      soilHealthIssues.push("Low organic matter");
    }
    if (data.electricalConductivity > 2.0) {
      soilHealthScore -= 15;
      soilHealthIssues.push("High salinity");
    }
    if (data.drainage === "poor") {
      soilHealthScore -= 10;
      soilHealthIssues.push("Poor drainage");
    }
    if (data.slope > 5) {
      soilHealthScore -= 5;
      soilHealthIssues.push("Steep slope");
    }
    if (data.soilDepth < 20) {
      soilHealthScore -= 10;
      soilHealthIssues.push("Shallow soil");
    }
    if (data.bulkDensity > 1.6) {
      soilHealthScore -= 10;
      soilHealthIssues.push("High bulk density");
    }
    ...
    if (nRatio < 0.8) {
      cropSuitabilityScore -= 15;
      suitabilityIssues.push("Nitrogen deficiency");
    }
    if (pRatio < 0.8) {
      cropSuitabilityScore -= 15;
      suitabilityIssues.push("Phosphorus deficiency");
    }
    if (kRatio < 0.8) {
      cropSuitabilityScore -= 15;
      suitabilityIssues.push("Potassium deficiency");
    }
    ...
    function getStatusText(measured: number, required: number) {
      ...
      if (ratio < 0.8)
        return `Deficient (${(required - measured).toFixed(2)} kg/ha needed)`;
    ...
    return {
      ...
      cropSuitability: {
        ...
        status:
          cropSuitabilityScore >= 80
            ? "Excellent"
            : cropSuitabilityScore >= 60
              ? "Good"
              : cropSuitabilityScore >= 40
                ? "Fair"
                : "Poor",
      },
      ph: {
  ...
  const handleAnalyze = async () => {
    ...
    await submitSoilAnalysis(soilData);
  };
  ...
  const submitSoilAnalysis = async (soilData: ExtendedSoilData) => {
    try {
      ...
      const payload = {
        soilParameters: {
          ph: { value: soilData.ph, status: getStatus(soilData.ph, 6.0, 7.5) },
          moisture: {
            value: soilData.moisture,
            status: getStatus(soilData.moisture, 30, 80),
          },
          nitrogen: {
            value: soilData.nitrogen,
            status: getStatus(soilData.nitrogen, 30, 80),
          },
          phosphorus: {
            value: soilData.phosphorus,
            status: getStatus(soilData.phosphorus, 25, 70),
          },
          potassium: {
            value: soilData.potassium,
            status: getStatus(soilData.potassium, 30, 75),
          },
          temperature: { value: soilData.temperature, unit: "celsius" },
          organicMatter: {
            value: soilData.organicMatter,
            status: getStatus(soilData.organicMatter, 1.0, 3.0),
          },
          electricalConductivity: {
        ...
        location: {
          ...
          // In a real app, you would get this from the user or a map API.
          coordinates: [84.9916, 24.7954], // [longitude, latitude] for Gaya
          address: "Gaya, Bihar, India",
        },
        // farm: "farmId" // optional: include if a farm is selected
      };
      ...
      if (response.ok && data.success) {
        ...
        console.log("Soil analysis submitted successfully:", data);
      } else {
        // handle error
        console.error("Soil analysis submission failed:", data);
      }
    } catch (error) {
      // handle error
                ...
                <CardContent className="space-y-6">
                  ...
                  <div>
                    <Label className="flex items-center space-x-2 mb-3">
                      <span>Plot Area</span>
                      <span className="text-sm text-gray-500">
                        ({soilData.plotArea} {soilData.areaUnit})
                      </span>
                    </Label>
                    <Input
                      type="number"
                    ...
                    <div className="flex gap-4 mt-2">
                      <label className="flex items-center gap-1">
                        <input
                          ...
                          onChange={() =>
                            setSoilData({ ...soilData, areaUnit: "hectare" })
                          }
                        />
                        Hectare
                      </label>
                      <label className="flex items-center gap-1">
                        <input
                          ...
                          onChange={() =>
                            setSoilData({ ...soilData, areaUnit: "acre" })
                          }
                        />
                        Acre
                      </label>
                  ...
                  <div>
                    ...
                    <select
                      ...
                      onChange={(e) =>
                        setSoilData({ ...soilData, district: e.target.value })
                      }
                          ...
                          <>
                            <div className="p-3 rounded-lg border bg-gray-50">
                              ...
                              <div className="grid grid-cols-2 md:grid-cols-4 gap-2 text-xs">
                                <div>
                                  ...
                                  <span
                                    style={{
                                      color: calcResults.status.nitrogen.color,
                                    }}
                                  >
                                    {calcResults.measured.nitrogen} /{" "}
                                    {calcResults.required.nitrogen} (
                                    {calcResults.status.nitrogen.text})
                                  </span>
                                </div>
                                <div>
                                  Phosphorus:{" "}
                                  <span
                                    style={{
                                      color:
                                        calcResults.status.phosphorus.color,
                                    }}
                                  >
                                    {calcResults.measured.phosphorus} /{" "}
                                    {calcResults.required.phosphorus} (
                                    {calcResults.status.phosphorus.text})
                                  </span>
                                </div>
                                <div>
                                  Potassium:{" "}
                                  <span
                                    style={{
                                      color: calcResults.status.potassium.color,
                                    }}
                                  >
                                    {calcResults.measured.potassium} /{" "}
                                    {calcResults.required.potassium} (
                                    {calcResults.status.potassium.text})
                                  </span>
                                </div>
                                <div>
                                  pH: {calcResults.measured.ph} (Range:{" "}
                                  {calcResults.required.minPh} -{" "}
                                  {calcResults.required.maxPh})
                                </div>
                              </div>
                            </div>
                            <div className="p-3 rounded-lg border bg-yellow-50">
                              ...
                              <div className="grid grid-cols-3 gap-2 text-xs">
                                <div>
                                  Nitrogen: {calcResults.deficiencies.nitrogen}{" "}
                                  kg/ha
                                </div>
                                <div>
                                  Phosphorus:{" "}
                                  {calcResults.deficiencies.phosphorus} kg/ha
                                </div>
                                <div>
                                  Potassium:{" "}
                                  {calcResults.deficiencies.potassium} kg/ha
                                </div>
                              </div>
                            </div>
                            <div className="p-3 rounded-lg border bg-blue-50">
                              ...
                              <div className="grid grid-cols-2 md:grid-cols-3 gap-2 text-xs">
                                <div>
                                  Urea:{" "}
                                  {calcResults.fertilizers.urea.perHa.toFixed(
                                    2,
                                  )}{" "}
                                  kg/ha (Total:{" "}
                                  {calcResults.fertilizers.urea.total.toFixed(
                                    2,
                                  )}{" "}
                                  kg, Cost: ₹
                                  {calcResults.fertilizers.urea.cost.toFixed(0)}
                                  )
                                </div>
                                <div>
                                  DAP:{" "}
                                  {calcResults.fertilizers.dap.perHa.toFixed(2)}{" "}
                                  kg/ha (Total:{" "}
                                  {calcResults.fertilizers.dap.total.toFixed(2)}{" "}
                                  kg, Cost: ₹
                                  {calcResults.fertilizers.dap.cost.toFixed(0)})
                                </div>
                                <div>
                                  MOP:{" "}
                                  {calcResults.fertilizers.mop.perHa.toFixed(2)}{" "}
                                  kg/ha (Total:{" "}
                                  {calcResults.fertilizers.mop.total.toFixed(2)}{" "}
                                  kg, Cost: ₹
                                  {calcResults.fertilizers.mop.cost.toFixed(0)})
                                </div>
                              </div>
                              <div className="mt-2 text-sm font-semibold">
                                Total Cost: ₹
                                {calcResults.fertilizers.totalCost.toFixed(0)}
                              </div>
                            </div>
                            <div className="p-3 rounded-lg border bg-green-50">
                              ...
                              <div>
                                Recommendation: {calcResults.ph.recommendation}
                              </div>
                            </div>
                          </>
                        )}
        ...
        <section className="py-10 bg-white">
          <div className="container-max section-padding">
            <h3 className="text-2xl font-bold mb-4 text-gray-900">
              Soil Analysis History (This Session)
            </h3>
                ...
                <tbody>
                  {history.map((entry, idx) => (
                    <tr key={entry.timestamp} className="text-center">
                      ...
                      <td className="px-3 py-2 border-b">
                        {entry.soilData.nitrogen}
                      </td>
                      <td className="px-3 py-2 border-b">
                        {entry.soilData.phosphorus}
                      </td>
                      <td className="px-3 py-2 border-b">
                        {entry.soilData.potassium}
                      </td>
                      <td className="px-3 py-2 border-b">
                        {entry.soilData.temperature}
                      </td>
                      <td className="px-3 py-2 border-b text-xs">
